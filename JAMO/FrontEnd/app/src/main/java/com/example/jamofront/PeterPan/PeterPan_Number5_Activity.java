...
import com.example.jamofront.databinding.PeterpanNumber5Binding;
...
public class PeterPan_Number5_Activity extends AppCompatActivity implements View.OnClickListener {

    private PeterpanNumber5Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private String URl = "http://3.35.123.120:5000/solved-tquestion" ;
    private String Answertext [] = {"화장실","비디오","녹차","초록색"};
    private int count = 0;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.peterpan_number5);
        binding = PeterpanNumber5Binding.inflate(getLayoutInflater());
        ...
        switch(v.getId())
        {
            ...
            case R.id.answer3_btn: {
                ...
                if (count == 1) {
                    ...
                    if (count == 1) {
                        sendData(Userinfo.id,"65","false",URl);
                    }
                }
            ...
            case R.id.answer4_btn:
            {
                ...
                if (count == 1) {
                    if (count == 1) {
                        sendData(Userinfo.id,"65","true",URl);
                        // Post 하기위한 조건문 들어갈 자리
                    }
                    // Post 하기위한 조건문 들어갈 자리
                }
                //
                Intent intent = new Intent(getApplicationContext(), select_peterpantype.class);
                showDialog("문제가 끝났습니다. 이제 다른 유형을 선택해보세요!", true, intent); // 정답 Dialog
                break;
            }
