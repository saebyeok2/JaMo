...
import com.example.jamofront.databinding.LittleprinceSelectChoose2Binding;
...
public class Littleprince_SelectChoose2_Activity extends AppCompatActivity implements View.OnClickListener {
    private LittleprinceSelectChoose2Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private int count = 0;
    private String URl = "http://3.35.123.120:5000/solved-tquestion";
    private String ProblemAnswer[] = {"ㄴ", "ㅏ", "ㅁ","ㅜ"};
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.littleprince_select_choose2);
        binding = LittleprinceSelectChoose2Binding.inflate(getLayoutInflater());
    ...
    private void CheckAnswer()
    {
        ...
        if(Arrays.equals(UserAnswer, ProblemAnswer))
        {
            ...
            Intent intent = new Intent(getApplicationContext(), Littleprince_SelectChoose3_Activity.class);
            if (count == 1) {
                sendData(Userinfo.id,"27","true", URl);
            }
            showDialog("정답입니다. 다음문제로 넘어갑니다.", true, intent);
        }
        else {
            if (count == 1) {
                sendData(Userinfo.id,"27","false", URl);
            }
