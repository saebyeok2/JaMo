...
import com.example.jamofront.databinding.PeterpanLength4Binding;
...
public class PeterPan_Length4_Activity extends AppCompatActivity implements View.OnClickListener {
    private PeterpanLength4Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private String URl = "http://3.35.123.120:5000/solved-tquestion" ;
    private String ProblemText [] = {"학교","세탁기"};
    private int count = 0;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.peterpan_length4);
        binding =  PeterpanLength4Binding.inflate(getLayoutInflater());
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            case R.id.answer1_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"54","false",URl);
            ...
            case R.id.answer2_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"54","true",URl);
                    // Post 하기위한 조건문 들어갈 자리
                }
                Intent intent = new Intent(getApplicationContext(), PeterPan_Length5_Activity.class);
