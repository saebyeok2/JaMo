...
import com.example.jamofront.databinding.RedhatLength5Binding;
...
public class RedHat_length5_Activity extends AppCompatActivity implements View.OnClickListener {
    private RedhatLength5Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private String URl = "http://3.35.123.120:5000/solved-tquestion" ;
    private String ProblemText [] = {"라디오","와이셔츠"};
    private int count = 0;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.redhat_length5);
        binding = RedhatLength5Binding.inflate(getLayoutInflater());
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            case R.id.answer2_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"95","true",URl);
            ...
            case R.id.answer1_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"95","false",URl);
                    // Post 하기위한 조건문 들어갈 자리
                }
