...
import com.example.jamofront.databinding.LittleprinceLength5Binding;
...
public class Littleprince_length5_Activity extends AppCompatActivity implements View.OnClickListener {
    private int count = 0;
    private ImageView popupMenuBtn;
    private LittleprinceLength5Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private String URl = "http://3.35.123.120:5000/solved-tquestion" ;
    private String ProblemText [] = {"아이스크림","해바라기"};
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding = LittleprinceLength5Binding.inflate(getLayoutInflater());
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            case R.id.answer1_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"15","true",URl);
            ...
            case R.id.answer2_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"15","false",URl);
                    // Post 하기위한 조건문 들어갈 자리
                }
