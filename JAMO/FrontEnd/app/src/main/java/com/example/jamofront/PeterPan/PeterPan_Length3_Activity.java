...
import com.example.jamofront.databinding.PeterpanLength3Binding;
...
public class PeterPan_Length3_Activity extends AppCompatActivity implements View.OnClickListener {
    private PeterpanLength3Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private String URl = "http://3.35.123.120:5000/solved-tquestion" ;
    private String ProblemText [] = {"그림자","학생"};
    private int count = 0;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.peterpan_length3);
        binding =  PeterpanLength3Binding.inflate(getLayoutInflater());
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            case R.id.answer2_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"53","false",URl);
            ...
            case R.id.answer1_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"53","true",URl);
                    // Post 하기위한 조건문 들어갈 자리
                }
                Intent intent = new Intent(getApplicationContext(), PeterPan_Length4_Activity.class);
