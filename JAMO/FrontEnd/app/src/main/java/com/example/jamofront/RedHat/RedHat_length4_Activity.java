...
import com.example.jamofront.databinding.RedhatLength4Binding;
...
public class RedHat_length4_Activity extends AppCompatActivity implements View.OnClickListener {
    private RedhatLength4Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private String URl = "http://3.35.123.120:5000/solved-tquestion" ;
    private String ProblemText [] = {"코트","노래방"};
    private int count = 0;
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.redhat_length4);
        binding = RedhatLength4Binding.inflate(getLayoutInflater());
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            case R.id.answer2_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"94","true",URl);
                    // Post 하기위한 조건문 들어갈 자리
                }
                Intent intent = new Intent(getApplicationContext(), RedHat_length5_Activity.class);
            ...
            case R.id.answer1_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"94","false",URl);
                    // Post 하기위한 조건문 들어갈 자리
                }
