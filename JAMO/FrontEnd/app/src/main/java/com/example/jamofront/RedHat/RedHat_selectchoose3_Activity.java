...
import com.example.jamofront.databinding.LittleprinceSelectChoose1Binding;
import com.example.jamofront.databinding.RedhatSelectchoose1Binding;
import com.example.jamofront.databinding.RedhatSelectchoose3Binding;
...
public class RedHat_selectchoose3_Activity extends AppCompatActivity implements View.OnClickListener {
    private RedhatSelectchoose3Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private int count = 0;
    private String URl = "http://3.35.123.120:5000/solved-tquestion";
    private String ProblemAnswer[] = {"ㅅ", "ㅣ", "ㄱ", "ㅁ", "ㅜ", "ㄹ"};
    private String UserAnswer[] = new String[6];
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.redhat_selectchoose3);
        binding = RedhatSelectchoose3Binding.inflate(getLayoutInflater());
    ...
    public void inputAnswerText()
    {
        switch (Answers)
        {
            ...
            case 5: {
                binding.answer6Btn.setText(Text);
                UserAnswer[5]=Text;
                Answers++;
                break;
            }
    ...
    private void CheckAnswer()
    {
        ...
        {
            if (count == 1) {
                sendData(Userinfo.id,"108","true", URl);
            }
            Intent intent = new Intent(getApplicationContext(), RedHat_selectchoose4_Activity.class);
            showDialog("정답입니다. 다음 문제로 넘어갑니다.", true, intent); // 정답 Dialog

            // 정답일시 Intent를 3초후에 실행

        }
        else if (!Arrays.equals(UserAnswer, ProblemAnswer))
        {
            if (count == 1) {
                sendData(Userinfo.id,"108","false", URl);
            };
