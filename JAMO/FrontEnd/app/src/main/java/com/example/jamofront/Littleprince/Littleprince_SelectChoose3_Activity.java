...
import com.example.jamofront.databinding.LittleprinceSelectChoose3Binding;
...
public class Littleprince_SelectChoose3_Activity extends AppCompatActivity implements View.OnClickListener {
    private LittleprinceSelectChoose3Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private int count = 0;
    private String URl = "http://3.35.123.120:5000/solved-tquestion";
    private String ProblemAnswer[] = {"ㅎ", "ㅜ", "ㅎ","ㅗ","ㅣ"};
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        setContentView(R.layout.littleprince_select_choose3);
        binding = LittleprinceSelectChoose3Binding.inflate(getLayoutInflater());
    ...
    private void CheckAnswer()
    {
        ...
        if(Arrays.equals(UserAnswer, ProblemAnswer))
        {
            ...
            Intent intent = new Intent(getApplicationContext(), Littleprince_SelectChoose4_Activity.class);
            if (count == 1) {
                sendData(Userinfo.id,"28","true", URl);
            }
            showDialog("정답입니다. 다음문제로 넘어갑니다.", true, intent);
        }
        else if(!Arrays.equals(UserAnswer, ProblemAnswer)) {
            if (count == 1) {
                sendData(Userinfo.id,"28","false", URl);
            }
