...
import com.example.jamofront.databinding.LittleprinceLength4Binding;
...
public class Littleprince_length4_Activity extends AppCompatActivity implements View.OnClickListener{
    private int count = 0;
    private ImageView popupMenuBtn;
    private LittleprinceLength4Binding binding;
    private SubmitTrTasek httpConn2 = SubmitTrTasek.getInstance();
    private String URl = "http://3.35.123.120:5000/solved-tquestion" ;
    private String ProblemText [] = {"의자","강아지"};
    ...
    protected void onCreate(@Nullable Bundle savedInstanceState) {
        ...
        binding = LittleprinceLength4Binding.inflate(getLayoutInflater());
    ...
    public void onClick(View v) {
        switch (v.getId())
        {
            case R.id.answer1_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"14","false",URl);
            ...
            case R.id.answer2_btn:
            {
                count += 1;
                if (count == 1) {
                    sendData(Userinfo.id,"14","true",URl);
                    // Post 하기위한 조건문 들어갈 자리
                }
                Intent intent = new Intent(getApplicationContext(), Littleprince_length5_Activity.class);
